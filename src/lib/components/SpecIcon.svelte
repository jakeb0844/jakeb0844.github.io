...
<button
    class="spec-icon-wrapper relative w-12 h-12 rounded-lg overflow-hidden transition-all duration-200 {selected ? 'ring-2 ring-yellow-400 scale-110' : 'opacity-75 hover:opacity-100'}"
    on:click={onClick}
...
>
    ...
    {:else}
        <div class="absolute inset-0 bg-black/30 flex items-center justify-center text-white/50">?</div>
    {/if}
</button>
