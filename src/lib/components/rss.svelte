...
<div class="w-full">
  ...
  <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">{title}</h2>
  {#if clientItems && clientItems.length}
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6">
      {#each clientItems as item}
        <div class="flex flex-col p-3 h-full rounded-lg border border-gray-200 dark:border-gray-700">
          <a  style="font-size: 12px;" class="text-base font-semibold text-blue-700 line-clamp-2 hover:underline dark:text-blue-400" href={item.link} rel="noopener noreferrer" target="_blank">{item.title}</a>
          {#if item.pubDate}
            <div style="font-size: 10px;" class="mt-1 text-gray-500">{new Date(item.pubDate).toLocaleString()}</div>
          {/if}
          <!-- {#if item.description}
