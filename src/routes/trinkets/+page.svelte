<script>
  ...
  $: if (selectedSpec) {
    ...
    currentTierList =
      trinketData.find((spec) => `${spec.class} ${spec.name}` === selectedSpec)
        ?.tierList ?? null;
  }
...
<div class="container mx-auto px-4 py-8">
  ...
  <div
    class="mb-8 p-4 rounded-xl border backdrop-blur-sm border-wow-gold/30 bg-wow-parchment/90 shadow-[0_2px_8px_rgba(0,0,0,0.08)]"
  >
    <form
      ...
      <div class="max-w-3xl mx-auto mt-6">
        <div class="flex items-center gap-3 mb-6">
          ...
          <a
            ...
            class="text-wow-navy hover:text-wow-teal">{selectedTrinket.name}</a
          >
          <button
        ...
        {#if trinketTierResults}
          {#each tiers as tier}
            <div class="mb-4">
              <h3 class="text-xl font-semibold mb-2 {tierColors[tier]}">
