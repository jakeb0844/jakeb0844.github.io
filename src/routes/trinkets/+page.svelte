<script>
    ...
    $: filteredSpecs = specs.filter(spec => 
        `${spec.className} ${spec.specName}`.toLowerCase().includes(searchTerm.toLowerCase())
    );
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="relative max-w-xl mx-auto mb-4">
            ...
            />
            <svg class="w-4 h-4 absolute left-2 top-3 text-white/50" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="tier-section">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {#each currentTierList[tier] || [] as trinket}
                                <div class="trinket-card bg-white/5 backdrop-blur-sm p-4 rounded-lg flex items-center space-x-4 hover:bg-white/10 transition-colors">
                                    <img src={trinket.iconUrl} alt={trinket.name} class="w-12 h-12" />
                                    <a
                                        href={trinket.href}
    ...
    {:else if selectedSpec}
        <p class="text-center text-gray-400">No tier list data available for {selectedSpec}</p>
    {:else}
        <p class="text-center text-gray-400">Select a spec to view its trinket tier list</p>
    {/if}
</div>