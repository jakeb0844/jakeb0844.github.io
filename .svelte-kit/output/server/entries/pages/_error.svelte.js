...
const Error$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  return `<div class="min-h-screen flex items-center justify-center w-full"><div class="text-center w-full px-6"> <div class="text-6xl font-bold text-red-600 dark:text-red-500 mb-4">${escape(
    $page.status
  )}</div>  <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-4">${
    $page.status === 404
      ? `The Page Has Been Consumed by the Void`
      : `A Wild Error Appears`
  }</h1>  <p class="text-xl text-gray-700 dark:text-gray-400 mb-8">${
    $page.status === 404
      ? `Looks like this page has been banished to the Twisting Nether. 
        Perhaps a mischievous imp has made off with it?`
      : `${escape($page.error?.message || "Something went wrong in Azeroth")}`
