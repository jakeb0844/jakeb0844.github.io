  ...
  return `${validate_component(Frame, "Frame").$$render(
    ...
    {},
    {
      default: () => {
        return `${
          img
            ? `<img${add_attribute("class", imgClass, 0)}${add_attribute(
                "src",
                img,
                0
              )} alt=""> <div${add_attribute("class", innerPadding, 0)}>${
...
const Rss = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<div class="mx-auto max-w-5xl"> ${
    (tickerItems && tickerItems.length) || (clientItems && clientItems.length)
      ? `<div class="overflow-hidden p-2 mb-4 text-sm bg-white rounded border border-gray-200 dark:border-gray-700 dark:bg-gray-800">${
          tickerItems && tickerItems.length
            ? `<a class="flex gap-2 items-center whitespace-nowrap"${add_attribute(
                "href",
                tickerItems[tickerIndex]?.link,
                0
              )} target="_blank" rel="noopener noreferrer"><span class="ticker-text line-clamp-1 svelte-mgkgu5">${escape(
                tickerItems[tickerIndex]?.title
              )}</span></a>`
            : ``
        }</div>`
      : ``
  } <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">${escape(
    title
  )}</h2> ${
    clientItems && clientItems.length
      ? `<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">${each(
          clientItems,
          (item) => {
            return `<div class="flex flex-col p-3 h-full text-sm rounded-lg border border-gray-200 dark:border-gray-700"><a class="text-base font-semibold text-blue-700 line-clamp-2 hover:underline dark:text-blue-400 svelte-mgkgu5"${add_attribute(
              "href",
              item.link,
              0
              ...
              item.pubDate
                ? `<div class="mt-1 text-xs text-gray-500">${escape(
                    new Date(item.pubDate).toLocaleString()
                  )}</div>`
                : ``
            } ${
              item.description
                ? `<div class="mt-2 line-clamp-3 svelte-mgkgu5"><!-- HTML_TAG_START -->${item.description}<!-- HTML_TAG_END --></div>`
                : ``
            } <div class="mt-3"><a class="text-xs text-blue-600 hover:underline dark:text-blue-300"${add_attribute(
              "href",
              item.link,
              0
            )} rel="noopener noreferrer" target="_blank">${escape(
      ...
      : `${`<div class="text-gray-600 dark:text-gray-300" data-svelte-h="svelte-kwfyjd">Loading news…</div>`}`
  } </div>`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<div class="min-h-screen"> <div class="relative h-[400px] mb-12 overflow-hidden" style="background-image: url('wow-background.jpg'); background-size: cover; background-position: center;" data-svelte-h="svelte-pfnfk2"></div> <div class="px-6 w-full">${validate_component(
    Rss,
    "Rss"
  ).$$render(
    $$result,
    ...
    {},
    {}
  )}</div>  <div class="px-6 w-full"> <div class="grid grid-cols-1 gap-6 mb-12 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">${validate_component(
    Card,
    "Card"
  ).$$render(
    $$result,
